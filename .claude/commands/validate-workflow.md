...
3. **Naming-Convention** — Wenn der Community-MCP-Server (`n8n-mcp-community`) verfügbar ist, dessen `validate_workflow` zusätzlich aufrufen — er kennt Node-Typen exakt. Falls nicht: Regex-Check `^[a-z][a-z0-9-]*$` auf `.name` (kein Env-Prefix mehr — Env via `config/env-mapping.yaml`).
